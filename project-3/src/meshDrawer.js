...
void main()
{
	...
	// diffuse lighting
	vec3 lightColor = vec3(1.0, 1.0, 1.0); // Assuming white light for simplicity
	diff = max(dot(normal, lightdir), 0.0) * 0.7; // 0.7 is a factor for light intensity

	// specular lighting
	vec3 cameraPos = vec3(0.0, 0.0, -1.0); // Camera position is defined at the [0,0,-1] in the HTML file
	vec3 viewDir = normalize(cameraPos - fragPos); 
	vec3 reflectDir = reflect(-lightdir, normal);
	float specularStrength = 0.5;
	spec = pow(max(dot(viewDir, reflectDir), 0.0), phongExp) * specularStrength;
